...
export default function Join() {
    ...
    function chgMember(e){
        ...
        setMember({...member});               //State 변수 복사 및 전달하여 리렌더링 유도
    }


    //비밀번호 확인 값 변경 시, 저장 변수 (서버 전송 X. 화면에서 처리를 위함.)
    const [memberPwRe, setMemberPwRe] = useState('');
    //비밀번호 확인값 onChange 호출 함수
    ...
    /* 아이디 유효성 체크 결과, 중복체크 결과를 저장할 변수
       0 : 검증 이전 상태 
       1 : 회원 가입 가능한 상태(유효성 검증 통과 && 중복체크 통과)
       2 : 유효성 체크 실패
       3 : 중복된 아이디가 존재하는 경우
    */
    ...
    function join(){
        if(idChk == 1 && pwChk == 1){ 
            ...
            .then(function(res){
                ...
                //res.data.resData == 회원 가입 결과 (true or false)
