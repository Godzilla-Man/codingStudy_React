...
function setInterceptors(instance) {
	...
	instance.interceptors.response.use(
		...
		function(response) { // 서버 응답이 정상적으로 도착한 경우        
			...
			//서버에서 HttpStatus.OK로 응답한 경우 ** 주석 추가할 것.
				...
				if(error.config.headers.refreshToken === undefined
					...
					return instance(options)
						   .then(function(res){
								if(res.data.resData != null){
									...
									originalRequest._retry = true; //무한루프 방지 차원에서 재 요청임을 저장
					...
					Swal.fire({
						...
						if(result.isConfirmed){
							...
							//history 도구를 이용해서, 브라우저 주소창을 변경하면, 이를 라우터가 감지하고 등록된 컴포넌트 중, path가 일치하는 컴포넌트로 전환(렌더링) 해줌.
